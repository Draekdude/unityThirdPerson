Shader "Hidden/TextureGraph/Blur"
{
	Properties
	{
		...
		CGINCLUDE
#define BLUR(radius)	float2 texelSize = _MainTex_TexelSize.xy;\
						float2 textureRes = _MainTex_TexelSize.zw;\
						float4 avgColor = float4(0,0,0,0);\
						float sampleCount = 0;\
						float2 uv = float2(0,0);\
						float2 centerPixel = i.uv * textureRes;\
						...
						{\
							...
							{\
								uv = i.uv + float2(x0 * texelSize.x, y0 * texelSize.y);\
								pixelPos = uv * textureRes;\
								d = distance(centerPixel, pixelPos);\
								clipMul = d <= radius;\
								avgColor += tex2D(_MainTex, uv) * clipMul;\
								sampleCount += 1 * clipMul;\
							}\
						}\
						avgColor = avgColor/sampleCount;\
						avgColor = float4(avgColor.rrr, 1)*_RRR + avgColor*(1-_RRR);\
						return avgColor;\
